...
class FirebaseGeoLocation {
  ...
  Future<void> setLocation(String key, List<double> position) {
    if (position[0] > 90 || position[0] < -90) {
      throw new ArgumentError.value(position[0], "latitude",
          "The latitude argument has to be a double between -90 and 90 (inclusive)");
    }
    if (position[1] > 180 || position[1] < -180) {
      throw new ArgumentError.value(position[1], "longitude",
          "The longitude argument has to be a double between -180 and 180 (inclusive)");
    }
  ...
  Future<void> removeLocation(String key) {
  ...
  void removeGeoQuery(GeoQuery query) {
    if (!queries.contains(query))
      throw new ArgumentError(
          "The query you are trying to delete is not attached to this object!");
